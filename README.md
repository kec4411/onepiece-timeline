...
イベントの実態は**すべて `events`** が持ち、`character_events`（中間テーブル: `character_id, event_id`）で
誰に紐づくかを表す。**`character_id = 0` は「世界の出来事」**（最上段の帯）、`character_id = N` は
そのキャラの節目で**生涯バー上に○マーカー**表示（1つのイベントを複数キャラ＋世界に紐づけ可）。
